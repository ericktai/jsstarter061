...
(function() {
  ...
  function createBaseString(ts, nonce, method, uri, host, port) {
    var nl = '\u000A';
    console.debug('Params passed into createBaseString:');
    console.debug({'ts': ts, 'method': method, 'uri': uri, 'host': host, 'port': port})
    return ts + nl + nonce + nl + method + nl + uri + nl + host + nl + port + nl + nl;
  }

  function generateMAC(method, id, key, hostWithPort, url) {
  	console.debug('Params passed into generateMAC:');
  	console.debug({ 'method': method, 'accessToken': id, 'key': key, 'hostWithPort': hostWithPort, 'url': url });
	...
	console.debug('AUTHORIZATION: ' +  'MAC id="' + id + '",ts="' + ts + '",nonce="' + nonce + '",mac="' + mac + '"');
